...
def pos_get(address: str):
    return DB.execute("""
      SELECT address,status,entry_ts,entry_price,stop_price,breakeven_price,high_since_entry,half_sold
      FROM paper_positions WHERE address=?""", (address,)).fetchone()

def pos_upsert(address: str, **kw: Any):
    cols = ["status","entry_ts","entry_price","stop_price","breakeven_price","high_since_entry","half_sold"]
